...
CREATE TABLE IF NOT EXISTS user_profiles (
    ...
    verification_type TEXT CHECK (verification_type IN ('verified', 'creator', 'admin', 'premium', 'email')),
    verified_at TIMESTAMP WITH TIME ZONE,
    last_full_name_change TIMESTAMP WITH TIME ZONE,
...
-- Create verification requests table with all required columns
CREATE TABLE IF NOT EXISTS verification_requests (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
    verification_type TEXT DEFAULT 'verified' CHECK (verification_type IN ('verified', 'creator', 'admin', 'premium')),
...
DROP TRIGGER IF EXISTS trigger_auto_friend_creators ON user_profiles;
...
DROP POLICY IF EXISTS "Users can manage own blocked list" ON blocked_users;
...
DROP POLICY IF EXISTS "Only creators can manage creators table" ON creators;
...
DROP POLICY IF EXISTS "Anyone can view premium users" ON premium_users;
CREATE POLICY "Anyone can view premium users" ON premium_users
    FOR SELECT USING (true);

DROP POLICY IF EXISTS "Users and admins can manage premium status" ON premium_users;
...
DROP POLICY IF EXISTS "Admins and creators can view all restrictions" ON user_restrictions;
...
DROP POLICY IF EXISTS "Admins and creators can manage restrictions" ON user_restrictions;
...
DROP POLICY IF EXISTS "Anyone can view early access features" ON early_access_features;
CREATE POLICY "Anyone can view early access features" ON early_access_features
    FOR SELECT USING (true);

DROP POLICY IF EXISTS "Only creators can manage early access features" ON early_access_features;
...
DROP POLICY IF EXISTS "Users can view own files and public files" ON storage.objects;
...
CREATE INDEX IF NOT EXISTS idx_messages_user_profiles ON messages(user_id);
CREATE INDEX IF NOT EXISTS idx_messages_recipient_profiles ON messages(recipient_id);
