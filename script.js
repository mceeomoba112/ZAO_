...
function updateVerificationSection() {
    ...
    // Find or create verification section in settings
    let verificationSection = document.getElementById('verification-section');
    if (!verificationSection) {
        // We'll add this to the settings modal in the HTML
        return;
    }

    // Only show verification details for admin-approved verification types
...
async function loadFriends() {
    try {
        ...
        if (!friendships || friendships.length === 0) {
            friendsList.innerHTML = '<div class="no-friends"><p>No friends yet. Start by sending friend requests!</p></div>';
            return;
        }
        ...
        console.log('Friends loaded:', sortedFriends);
    } catch (error) {
...
function getVerificationBadge(user) {
    ...
    // Exclude email verification - only show admin-approved badges
    ...
    switch (user.verification_type) {
        ...
        case 'verified':
            ...
            break;
        default:
            // Don't show badge for unknown verification types
            return '';
    }
...
function selectUser(user) {
    ...
    // Update chat header with verification badge
    const chatHeader = document.querySelector('.chat-header h2');
    const verificationBadge = getVerificationBadge(user);
    chatHeader.innerHTML = `${user.full_name} ${verificationBadge}`;
    ...
    try {
        ...
        if (error) {
            ...
            if (error.code === 'PGRST116') {
                ...
                showError('Permission error. Please try logging in again.', 'error');
                return;
        ...
        if (!data || data.length === 0) {
            ...
            const { data: profiles, error: profileError } = await supabase
                .from('user_profiles')
                .select('full_name, username, avatar_url, user_id, is_verified, verification_type')
                .in('user_id', userIds);
...
// Subscribe to real-time messages with enhanced features
                ...
                if (message.user_id === currentUser.id || message.recipient_id === currentUser.id) {
                    ...
                    if (activeChat && 
                        ((message.user_id === activeChat.user_id && message.recipient_id === currentUser.id) ||
                         (message.user_id === currentUser.id && message.recipient_id === activeChat.user_id))) {
                        ...
                        const { data: fullMessage } = await supabase
                            ...
                            .select(`
                                *,
                                user_profiles(full_name, username, is_verified, verification_type)
                            `)
                            .eq('id', message.id)
                            .single();
                    ...
                    if (message.recipient_id === currentUser.id && Notification.permission === 'granted') {
                        ...
                        new Notification(`New message from ${senderProfile?.full_name || 'Someone'}`, {
...
// Enhanced send message with real-time sync
...
async function requestVerification() {
    try {
        ...
        // Try to check existing verification requests
        try {
            ...
            if (verificationError && verificationError.code !== 'PGRST116') {
                // Table exists but other error occurred
                console.error('Error checking verification status:', verificationError);
                
                // If it's a permission error, handle gracefully
                if (verificationError.code === '42501' || verificationError.message.includes('permission denied')) {
                    showError('Verification requests are handled by administrators only. Please contact support directly.', 'info');
                    return;
                }
            ...
            if (verificationData) {
                if (verificationData.status === 'pending') {
                    showError('Your verification request is already pending admin approval.');
                } else if (verificationData.status === 'approved') {
                    showError('Your account is already verified!');
                } else { // rejected
                    showError('Your previous verification request was rejected. Only administrators can grant verification status.');
                }
                return;
            }

            // Try to insert new verification request
            const { error } = await supabase
                .from('verification_requests')
                .insert([
                    {
            ...
            showError('Verification request submitted! Only administrators can approve verification requests.', 'success');

        } catch (tableError) {
            // Table doesn't exist or other connection issue
            console.log('Verification requests table not accessible:', tableError);
            showError('Verification requests are currently handled manually by administrators. Please contact support directly.', 'info');
        }
...
function displayMessage(message) {
    ...
    // Add read status if applicable
    let readStatusHtml = '';
    if (message.user_id === currentUser.id && message.is_read) {
        readStatusHtml = '<div class="read-status">✓✓</div>';
    }
